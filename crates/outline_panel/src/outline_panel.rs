    ...
    fn update_fs_entries(
        ...
        self.fs_entries_update_task = cx.spawn_in(window, async move |outline_panel, cx| {
            ...
            let Ok((
                ...
                let repo_snapshots = git_store.read(cx).repo_snapshots(cx);
                ...
                for excerpt in snapshot.excerpts() {
                    ...
                    buffer_excerpts
                        .entry(buffer_id)
                        .or_insert_with(|| {
                            let status = git_store.read(cx).status_for_buffer_id(buffer_id, cx);
...
mod tests {
    ...
    use db::indoc;
    use editor::{PathKey, RestoreOnlyUnstagedDiffHunkDelegate};
        ...
        editor.update_in(cx, |editor, window, cx| {
            ...
            assert_eq!(
                editor
                    ...
                    .read(cx)
                    .snapshot(cx)
                    .anchor_to_buffer_anchor(scroll_anchor)
                    .map(|(anchor, _)| anchor.buffer_id),
                Some(buffer_a_id),
            ...
            buffer.read_with(cx, |buffer, cx| {
                ...
                assert_eq!(
                    project
                        ...
                        .git_store()
                        .read(cx)
                        .status_for_buffer_id(buffer.remote_id(), cx),
                    None,
                );
            });
        }
        ...
        let (editor, diffs) = workspace.update_in(cx, |workspace, window, cx| {
            ...
            let editor = cx.new(|cx| {
                ...
                editor.set_diff_hunk_delegate(
                    Some(Arc::new(RestoreOnlyUnstagedDiffHunkDelegate)),
                    cx,
                );
