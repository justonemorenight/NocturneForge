...
/// Historical reason a persisted task was stopped by an older runtime budget.
/// New runs retain usage telemetry but do not create budget stops.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum BudgetExceeded {
    /// Retained only so historical persisted runs remain readable.
    ...
    pub fn error_class(&self) -> crate::verification::ErrorClass {
        match self {
            ...
            Self::ToolCallBudgetExceeded { .. } => {
                crate::verification::ErrorClass::ToolCallBudgetExceeded
            }
        }
    }
...
/// Live usage counters shared by the executor and Activity Center telemetry.
...
/// Rolling task usage plus a legacy stop reason when restoring older snapshots.
...
impl TaskExecutionReporter {
    ...
    /// Reports a tool call about to start for telemetry and live activity UI.
    pub fn report_tool_call_started(&self, tool: impl Into<String>) {
        if !self.is_current() {
            return;
        }
    ...
    pub fn report_total_tokens(&self, total_tokens: u64) {
        let mut usage = self.usage.write();
...
mod tests {
    ...
    fn stale_attempt_reporter_does_not_overwrite_live_worker_metadata() {
        ...
        let reporter = TaskExecutionReporter::new(RunId::new(), task_id.clone(), None, None)
