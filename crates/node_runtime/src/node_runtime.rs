...
impl ManagedNodeRuntime {
    ...
    async fn install_if_needed(http: &Arc<dyn HttpClient>) -> Result<Self> {
        ...
        // Reset the npm cache on every runtime initialization so repeated agent/server
        // installs cannot grow the cache without bound.
...
/// Resolves the executable declared by an npm package installed in a local
/// `node_modules` tree. Keeping this resolution local lets registry agents run
/// the installed binary directly instead of invoking `npm exec`, which would
/// maintain a second, unbounded npm cache.
