...
impl AgentPanel {
    ...
    pub fn load_agent_thread(
        ...
        if let BaseView::AgentThread { conversation_view } = &self.base_view {
            if conversation_view.read(cx).thread_id == thread_id {
                crate::thread_transcript_search::ThreadTranscriptSearchStore::
        ...
        {
            let conversation_view = draft.clone();
            self.set_base_view(
                BaseView::AgentThread {
                    conversation_view: draft,
                },
        ...
        if let Some(conversation_view) = self.retained_threads.remove(&thread_id) {
            ...
            crate::thread_transcript_search::ThreadTranscriptSearchStore::
                activate_pending_for_conversation(&pending_navigation_view, window, cx);
            return;
        }
