        ...
        cx.spawn(async move |view, cx| {
            ...
            let parsed_csv = cx
                .background_spawn(async move {
                    from_buffer_with_delimiter(&buffer_snapshot, delimiter)
                })
                .await;
...
fn parse_csv_with_positions(
    ...
    while let Some(ch) = chars.next() {
        ...
        match ch {
            ...
            character if character == delimiter && !in_quotes => {
                // Field separator
...
Jane,"Simple name""#;
    ...
    fn test_tsv_parsing() {
        ...
        let (parsed_cells, _) = parse_csv_with_positions(data, '\t');
