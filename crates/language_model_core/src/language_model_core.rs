...
pub enum ProviderErrorCategory {
    ...
    /// The request body exceeded a provider or gateway size limit, rather
    ...
    pub fn from_http_status(status: StatusCode, message: &str) -> Self {
        match status {
            ...
            StatusCode::PAYLOAD_TOO_LARGE => match parse_prompt_too_long(message) {
                ...
                None => Self::RequestPayloadTooLarge,
            },
...
mod tests {
    ...
    #[test]
    fn test_from_http_status_distinguishes_payload_size_from_context_overflow() {
        let payload_error = LanguageModelCompletionError::from_http_status(
            String::from("OpenAI").into(),
        ...
        assert!(matches!(
            payload_error,
            LanguageModelCompletionError::ProviderRejection {
                category: ProviderErrorCategory::RequestPayloadTooLarge,
                ..
            }
        ));
