...
#[gpui::test]
async fn test_subagent_continues_past_context_window_warning(cx: &mut TestAppContext) {
    init_test(cx);
    cx.update(|cx| {
        LanguageModelRegistry::test(cx);
    });
    ...
    // The subagent must remain running until the model finishes the turn.
    thread.read_with(cx, |thread, cx| {
        assert!(
            thread
                .running_subagent_ids(cx)
                .contains(&subagent_session_id),
            "subagent should continue past the advisory context warning"
        );
    });
    ...
    assert!(
        ...
        "tool output should not contain the removed context warning, got:\n{markdown}"
    );
