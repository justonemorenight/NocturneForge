    ...
    fn save_thread(
        ...
        let draft_prompt = if capture_live_draft {
            session
                .acp_thread
                .upgrade()
                .and_then(|acp_thread| acp_thread.read(cx).draft_prompt_snapshot(cx))
                .or_else(|| session.draft_prompt(cx))
        } else {
            session.draft_prompt(cx)
        };
                                ...
                                match request_result {
                                    ...
                                    Err(error) => {
                                        ...
                                        if response
                                            ...
                                            .is_err()
                                        {
                                            log::debug!("elicitation receiver was dropped");
                                        }
                                    }
                                }
                            }
...
mod internal_tests {
    ...
    fn set_draft_and_save(
        ...
        agent.update(cx, |agent, cx| {
            agent.save_thread(thread.clone(), true, cx);
        });
    }
}
