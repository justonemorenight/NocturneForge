...
async fn untracked_files_for_checkpoint(git: &GitBinary) -> Result<Vec<String>> {
    ...
    // Pass the checkpoint-specific excludes to Git without mutating
    // `.git/info/exclude`, whose writes trigger a repository rescan.
    ...
    Ok(futures::future::join_all(size_checks)
        .await
        .into_iter()
        .flatten()
        .collect())
