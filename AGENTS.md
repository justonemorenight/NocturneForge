...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-12 (commit 8f73373). Confidence: 100%.
...
- `crates/editor/src/editor.rs` — 50 bug fixes, last fix 6 weeks ago (bug magnet); 22 commits/90d
- `crates/editor/src/editor_tests.rs` — 38 bug fixes, last fix 8 weeks ago (bug magnet); 25 commits/90d
- `crates/agent_ui/src/agent_panel.rs` — 33 bug fixes, last fix 8 weeks ago (bug magnet); 26 commits/90d
- `crates/agent_ui/src/conversation_view/thread_view.rs` — 24 bug fixes, last fix 8 weeks ago (bug magnet); 44 commits/90d
- `crates/sidebar/src/sidebar.rs` — 30 bug fixes, last fix 3 months ago (bug magnet); 2 commits/90d

### Code health
Three co-equal signals: defect risk 6.83/10 avg, hotspot health 6.06/10 (stable), worst `crates/acp_thread/src/acp_thread.rs` at 5.15/10 · maintainability 5.09/10 · performance risk 318 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `crates/open_ai/src/responses.rs` — nested complexity (stream_response_with_body) — impact −1.8
- `crates/language_models/src/provider/ollama.rs` — nested complexity (to_ollama_request) — impact −1.5
- `crates/acp_thread/src/mention.rs` — nested complexity (parse) — impact −1.4
- `crates/edit_prediction_ui/src/rate_prediction_modal.rs` — god class (RatePredictionsModal) — impact −1.3
- `crates/extensions_ui/src/extensions_ui.rs` — god class (ExtensionsPage) — impact −1.2
